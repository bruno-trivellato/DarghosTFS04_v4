...
function autoBroadcast()
	...
	local messages = {
		...
		,{type = "pvp-channel", text = "Gosta de PvP? Participe de partidas com outros jogadores, ganhe recompensas, conquiste façanhas na Battleground! Guarde seu lugar na fila para a proxima partida com o comando '!bg entrar' agora mesmo!", chance = 40}
	...
	for k,v in pairs(messages) do
		...
		if(m <= v.chance) then
			if(v.type ~= nil and v.type == "pvp-channel") then
				broadcastChannel(CUSTOM_CHANNEL_PVP, v.text)
				break
			else

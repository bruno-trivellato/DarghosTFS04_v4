...
function NpcTasks:responseTask(cid)
	...
	if(cid ~= nil) then
		...
		if(self.cid ~= nil) then
			...
			consoleLog(T_LOG_WARNING, getNpcName(), "NpcTasks:responseTask", "No valid creature found.", {npc=getNpcName()})
		end
	end
	
	local startedTask = getPlayerStorageValue(cid, sid.TASK_STARTED)	
	consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "A player talking with NPC about tasks...", {npc=getNpcName(), player=getCreatureName(self.cid), startedTask=startedTask})
	
	-- verificamos se uma possivel task iniciada pelo player � uma das tasks do NPC
	if(startedTask ~= STORAGE_NULL and isInArray(self.registeredTasks, startedTask) == FALSE) then
		-- se n�o for, � chamado um pedido de descri��o, que deve mostrar
		-- os requerimentos para a task...
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "The player has an started task that is not from this NPC.")
		
		if(self:sendTaskDescription()) then
			consoleLog(T_LOG_ERROR, getNpcName(), "NpcTasks:responseTask", "Player with a started task and this is not from the NPC has received description.", {player=getCreatureName(self.cid), startedTask=startedTask})
		end
		
		return
	end
	
	if(startedTask == STORAGE_NULL) then
		...
		elseif(state == taskStats.STARTED) then
			...
			if(self:checkStatus()) then
				consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "All is ok! The player has done all requirements to complete task! We can now give the reward to him.")
				self:completed()
			end
		end		
	end
...
function NpcTasks:sendTaskDescription()
	...
	for k, v in pairs(self.registeredTasks) do	
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "NPC will verify if the player know something about our tasks.", {taskid=v})
		if(getPlayerStorageValue(self.cid, v) == taskStats.NONE) then
			consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Yeah... Player not know nothing about this task.")
			self:loadTask(v)
			
			local requiredTask = self.taskConf.requiredTask 
			
			if(requiredTask ~= nil) then
				consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Oh, this task needs requirements to start, we need check it.")
							
				if(getPlayerStorageValue(self.cid, requiredTask) ~= taskStats.COMPLETED) then
					consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Haha! The player not have necessary requirements, we will talk to him... :(")
					self.dialog:say(self.taskConf.dialogs.requireTask, self.cid)
					return false
				end
			end
			
			consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "All is ok! Now we can talk to player about our task...")
			self.dialog:say(self.taskConf.dialogs.description, self.cid)
			return true
		end
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Oh shit! The player already know about this task... We go to next (if is necessary)")
	end
	...
	self.dialog:say("Sorry, no more tasks available for you...", self.cid)
	return false	
end

function NpcTasks:sendTaskObjectives()

	consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Good! The player want help us! We will talk to him the task objetives!")
	local objective = self.taskConf.dialogs.taskObjectives

	if(type(objective) ~= "table") then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "The objectives is half, only one message needs!")
		self.dialog:say(objective, self.cid)
	else
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "The objectives is long, " .. # objective .. " messages are needs!")
		local isFirst = true
...
function NpcTasks:sendTaskStart()
	...
	consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Ok, the player has started our task! Good luck!", {player=getCreatureName(self.cid), startedTask=self.currentTask})
...
function NpcTasks:checkStatus()
	...
	if(not (tasks.hasStartedTask(self.cid))) then
		consoleLog(T_LOG_ERROR, getNpcName(), "NpcTasks:checkStatus", "Player not have an started task.", {player=getCreatureName(self.cid)})
		return false
	end
	...
	if(not task:loadById(self.currentTask)) then
		consoleLog(T_LOG_ERROR, getNpcName(), "NpcTasks:checkStatus", "The task initialized by player can not be loaded.", {player=getCreatureName(self.cid), taskid=self.currentTask})
		return false
	end
	...
	if(not (self:checkKills(task)) ) then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Oh shit! The player not have kill all monsters needed to complete task... We need talk to him it.")
		self.dialog:say(self.taskConf.dialogs.taskIncomplete, self.cid)
		return false
	end
	
	if(not (task:removeRequiredItems())) then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Oh shit! The player not have all required items to complete task... We need talk to him it.")
		self.dialog:say(self.taskConf.dialogs.taskIncomplete, self.cid)
		return false	
	end
...
function NpcTasks:completed()
	...
	if(not (tasks.hasStartedTask(self.cid))) then
		consoleLog(T_LOG_ERROR, getNpcName(), "NpcTasks:completed", "Player not have an started task.", {player=getCreatureName(self.cid)})
		return false
	end
	...
	if(task:getState() == taskStats.COMPLETED) then
		consoleLog(T_LOG_ERROR, getNpcName(), "NpcTasks:completed", "This player has already completed the task! Check It!", {player=getCreatureName(self.cid), taskid=self.currentTask})
		return false
	end
	...
	function giveRewards(task) 
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Giving the reward with an special delay! Is is beautiful!")
	...
	if(self.taskConf.events ~=  nil) then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Not ends! Apparently we have an event! We will check it...")
		if(self.taskConf.events.onComplete ~= nil) then
			consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Yes! We have an onComplete event!")
...
function NpcTasks:eventOnComplete(infos)
	...
	if(infos.type == nil) then
		consoleLog(T_LOG_WARNING, getNpcName(), "NpcTasks:eventOnComplete", "Event type not found.", {player=getCreatureName(self.cid), taskid=self.currentTask})
...
function NpcTasks:onCompleteConfirm()
	...
	consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Okey, the player has interacted with our onComplete event!")

	if(self.taskConf.events == nil or self.taskConf.events.onComplete == nil) then
		consoleLog(T_LOG_WARNING, getNpcName(), "NpcTasks:onCompleteConfirm", "Task dont have any event.", {player=getCreatureName(self.cid), taskid=self.currentTask})
		return
	end
	...
	if(infos.onConfirm == nil)  then
		consoleLog(T_LOG_WARNING, getNpcName(), "NpcTasks:onCompleteConfirm", "Unknown onConfirm event.", {player=getCreatureName(self.cid), taskid=self.currentTask})
		return
	end
	
	if(infos.onConfirm == "action") then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "The solution of event is an action...")
    ...
    self.npcSystem:setTopic(self.cid, 0)
		...
		function teleportPlayer(cid, destPos)
			...
			consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Ok, teleport with delay done!")
		end
	...
	if(infos.onConfirmText ~= nil) then
		consoleLog(T_LOG_NOTIFY, getNpcName(), "NpcTasks:responseTask", "Okey, we have an final message to player!")

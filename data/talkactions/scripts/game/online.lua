...
function onlinePrivileged(cid, words, param)
        ...
        local ping_table = {
          { minor = 1, major = 40, count = 0}
          ,{ minor = 41, major = 80, count = 0}
          ,{ minor = 81, major = 120, count = 0}
          ,{ minor = 121, major = 180, count = 0}
          ,{ minor = 181, major = 220, count = 0}
          ,{ minor = 221, major = 300, count = 0}
          ,{ minor = 301, major = 400, count = 0}
          ,{ minor = 401, major = 500, count = 0}
          ,{ minor = 501, count = 0}
        }
        
        function addPing(value)
          for k,v in pairs(ping_table) do
            if(value >= v.minor and (v.major == nil or value <= v.major)) then
              ping_table[k].count = ping_table[k].count + 1
              break
            end
          end
        end
        
        function drawPingTable()
          local string = ""
          
          for k,v in pairs(ping_table) do  
            if(v.major == nil) then
              string = string .. "[" .. v.minor .. "+ : " .. v.count .. "]"
            else
              string = string .. "[" .. v.minor .. "~" .. v.major .. " : " .. v.count .. "]\n"
            end
          end
        ...
        doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Ping statistics (by range):\n".. drawPingTable())

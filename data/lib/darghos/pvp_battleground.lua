...
function pvpBattleground.getInformations()
	...
	msg = msg .. "Este � um sistema de PvP do Darghos baseado no classico Capture the Flag, e o objetivo � seu time capturar a bandeira advers�ria e entregar-la em sua base 3 vezes. A partida tem dura��o de at� 15 minutos "
	msg = msg .. "se ao final do tempo nenhum time tiver capturado 3 vezes a bandeira adversaria a vit�ria � concedida ao com maior numero de bandeiras capturadas, e empate no caso de igualdade de bandeiras\n\n"
	msg = msg .. "Aos participantes do time vencedor � concedido uma quantidade de pontos de experiencia, dinheiro, rating e honra que voc� poder� usar para trocar por itens uteis com alguns NPCs!\n\n"
...
function pvpBattleground.broadcastLeftOnePlayer()
	...
	local messages = {
		"Quer ganhar bonus de experiencia, dinheiro e outras recompensas se divertindo com PvP no classico Capture The Flag? Participe da proxima battleground! Restam ap�nas mais um para fechar os times " .. BG_CONFIG_TEAMSIZE .. "x" .. BG_CONFIG_TEAMSIZE .. "! -> !bg entrar",
		"Restam ap�nas mais um jogador para fechar os times " .. BG_CONFIG_TEAMSIZE .. "x" .. BG_CONFIG_TEAMSIZE .. " para a proxima partida de Battleground! Ganhe recompensas! Ao morrer nada � perdido! Divirta-se! -> !bg entrar",
		"Gosta de PvP misturado com objetivos como o classico Capture The Flag? Prove seu valor! Restam ap�nas mais um jogadore para fechar os times " .. BG_CONFIG_TEAMSIZE .. "x" .. BG_CONFIG_TEAMSIZE .. " para a proxima Battleground! -> !bg entrar",
		"N�o conhece o sistema de Battlegrounds? Conhe�a agora! Falta ap�nas voc� para o proxima batalha 6x6! N�o h� perdas nas mortes, ajude o time na busca a bandeira advers�ria rumo a vitoria e ganhe recompensas! -> !bg entrar",
	}
...
function pvpBattleground.onEnter(cid)
	...
	if(doPlayerIsInArena(cid)) then
		doPlayerSendCancel(cid, "Voc� n�o pode entrar na battleground enquanto estiver dentro de uma Arena.")
		return false		
	end
	...
	if(not BATTLEGROUND_CAN_NON_PVP and not doPlayerIsPvpEnable(cid)) then
		doPlayerSendCancel(cid, "So � permitido a jogadores Agressivos a participarem de Battlegrounds.")
		return false
	end	
        
        local ring = getPlayerSlotItem(cid, CONST_SLOT_RING)
        if(ring.uid ~= 0) then
                doPlayerSendCancel(cid, "Para entrar em uma Battleground voc� n�o pode estar com nenhum anel equipado. Remova qualquer anel equipado e tente novamente.")
                return false
        end
	...
	if(ret == BG_RET_CAN_NOT_JOIN) then
		doPlayerSendCancel(cid, "Voc� abandonou uma battleground e foi marcado como desertor, e n�o poder� entrar em outra durante 25 minutos.")
		return false
	end	
...
function pvpBattleground.putFlag(team_id, pos, dropping)
	...
	-- n�o iremos deixar colocar a flag no mesmo tile q estiver uma escada ou nos spawns das bandeiras...
...
function pvpBattleground.onLeaveSpawnArea(cid)
	...
	if(lastTeamDeath ~= -1 and lastTeamDeath + BG_LEAVE_SPAWN_INTERVAL > os.time()) then
		...
		doPlayerSendCancel(cid, "Voc� podera sair em " .. leftSeconds .. " segundos.")
		return false	
	end
...
function pvpBattleground.onReportIdle(cid, idle_player)
	...
	if(hasCondition(idle_player, CONDITION_INFIGHT)) then
		pvpBattleground.sendPlayerChannelMessage(cid, "Este jogador est� em combate portanto n�o pode estar inativo.")
		return		
	end

...
void IOLoginData::removePremium(Account account)
{
	...
	{
		uint32_t days = (uint32_t)std::ceil((float)((timeNow - account.lastDay) / 86400));
		if(days > 0)
		{
...
bool IOLoginData::generateSpoofList(SpoofList& spoofList){
    ...
    query << "SELECT `id`, `name`, `account_id` FROM `players` WHERE `level` >= 20 AND `level` <= 80 AND `real_lastlogin` <= UNIX_TIMESTAMP() - (60 * 60 * 24 * 4) AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	if(rankId > 0)
	{
		query.str("");
#ifdef __DARGHOS_CUSTOM__
		query << "SELECT `guild_ranks`.`name` AS `rank`, `guild_ranks`.`guild_id` AS `guildid`, `guild_ranks`.`level` AS `level`, `guilds`.`name` AS `guildname` FROM `guild_ranks`, `guilds` WHERE `guild_ranks`.`id` = " << rankId << " AND `guild_ranks`.`guild_id` = `guilds`.`id` AND `guilds`.`status` = 1 LIMIT 1";
#else
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert)
{
	...
	for(ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId)
	{
		...
		std::stringstream ss;
		ss << player->getGUID() << ", " << it->first << ", " << runningId << ", " << item->getID() << ", " << (int32_t)item->getSubType() << ", " << db->escapeBlob(attributes, attributesSize).c_str();
...
bool IOLoginData::updateAfkStatus(uint32_t guid, bool afk)
{
	...
	query << "UPDATE `players` SET `afk` = " << value << " WHERE `id` = " << guid << db->getUpdateLimiter();
	return db->query(query.str());
}
#endif

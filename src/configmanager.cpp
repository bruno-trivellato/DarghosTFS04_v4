...
bool ConfigManager::load()
{
	...
	m_confNumber[RATE_STAMINA_LOSS] = getGlobalNumber("rate_stamina_loss", 1);
	m_confNumber[RATE_STAMINA_GAIN] = getGlobalNumber("rate_stamina_gain", 334);
	m_confNumber[SLOW_RATE_STAMINA_GAIN] = getGlobalNumber("slow_rate_stamina_gain", 84);
	m_confNumber[STAMINA_EXTRA_EXPERIENCE_DURATION] = getGlobalNumber("stamina_extra_experience_duration", 60 * 60 * 1000);
	m_confBool[STAMINA_EXTRA_EXPERIENCE_ONLYPREM] = getGlobalBool("stamina_extra_experience_onlyprem", true);
	m_confDouble[STAMINA_EXTRA_EXPERIENCE_RATE] = getGlobalDouble("stamina_extra_experience_rate", 0.5);

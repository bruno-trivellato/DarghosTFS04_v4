            ...
            if(msg.getString(4) == "info")
			{
				if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false))
				{
					if(Status* status = Status::getInstance())
					{
                        ...
                        if(msg.getLength() > msg.getBufferPosition())
                            sendPlayers = msg.get<uint8_t>() == 0x01;

                        output->addString(status->getStatusString(sendPlayers));
					}
...
void Status::getInfo(uint32_t requestedInfo, OutputMessage_ptr output, NetworkMessage& msg) const
{
	...
	if(requestedInfo & REQUEST_MISC_SERVER_INFO)
	{
        ...
        output->add<uint32_t>((uint32_t)(uptime >> 32));
        output->add<uint32_t>((uint32_t)(uptime));
	}

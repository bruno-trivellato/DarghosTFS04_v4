...
{
    double shareExperience = static_cast<double>(std::ceil(((static_cast<double>(experience) / (memberList.size() + 1)) + (static_cast<double>(experience) * extraExpRate))));
    double temp_share = shareExperience;

    leader->onGainSharedExperience(temp_share, fromMonster, multiplied);
    for(PlayerVector::iterator it = memberList.begin(); it != memberList.end(); ++it)
    {
        temp_share = shareExperience;
        (*it)->onGainSharedExperience(temp_share, fromMonster, multiplied);
    }
}

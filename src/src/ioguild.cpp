...
bool IOGuild::getGuildId(uint32_t& id, const std::string& name)
{
	...
	query << "SELECT `id` FROM `guilds` WHERE `name` " << db->getStringComparer() << db->escapeString(name) << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " AND `status` = 1 LIMIT 1";
#else
...
bool IOGuild::getGuildById(std::string& name, uint32_t id)
{
	...
	DBQuery query;
#ifdef __DARGHOS_CUSTOM__
	query << "SELECT `name` FROM `guilds` WHERE `id` = " << id << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " AND `status` = 1 LIMIT 1";
#else
...
bool IOGuild::guildExists(uint32_t guild)
{
	...
	DBQuery query;
#ifdef __DARGHOS_CUSTOM__
	query << "SELECT `id` FROM `guilds` WHERE `id` = " << guild << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " AND `status` = 1 LIMIT 1";
#else
...
void IOGuild::frag(Player* player, uint64_t deathId, const DeathList& list, bool score)
{
	...
	{
		#ifdef __DEBUG__
		std::cerr << "[Notice - IOGuild::frag] Unable to attach war frag to player " << player->getName() << "." << std::endl;
		#endif
		return;
	}

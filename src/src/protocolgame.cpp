...
{
    //dispatcher thread
    std::cerr << "[GAME] Starting login process for character '" << name << "' (account ID: " << id << ")" << std::endl;
    
    PlayerVector players = g_game.getPlayersByName(name);
    Player* _player = NULL;
    if(!players.empty())
    {
        std::cerr << "[GAME] Found " << players.size() << " existing player(s) with name '" << name << "'" << std::endl;
    ...
    {
        std::cerr << "[GAME] Creating new player instance for '" << name << "'" << std::endl;
        ...
        {
            std::cerr << "[GAME] Failed to load player data for '" << name << "'" << std::endl;
            disconnectClient(0x14, "Your character could not be loaded.");
            return false;
        }
        
        std::cerr << "[GAME] Player data loaded successfully for '" << name << "'" << std::endl;
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
    ...
    std::cerr << "[GAME] Character login attempt from IP: " << convertIPAddress(clientIp) << std::endl;

    if(g_game.getGameState() == GAMESTATE_SHUTDOWN)
    {
        std::cerr << "[GAME] Connection rejected - server shutting down" << std::endl;
        getConnection()->close();
        return false;
    }
    ...
    std::cerr << "[GAME] Client version: " << version << ", OS: " << operatingSystem << std::endl;
    if(!RSA_decrypt(msg))
    {
        std::cerr << "[GAME] RSA decryption failed for IP: " << convertIPAddress(clientIp) << std::endl;
        getConnection()->close();
        return false;
    }
    
    std::cerr << "[GAME] RSA decryption successful" << std::endl;
    ...
    std::cerr << "[GAME] Account: '" << name << "', Character: '" << character << "', GM: " << (gamemaster ? "yes" : "no") << " from IP: " << convertIPAddress(clientIp) << std::endl;

    msg.skipBytes(6); //841- wtf?
    if(version < CLIENT_VERSION_MIN || version > CLIENT_VERSION_MAX)
    {
        std::cerr << "[GAME] Invalid client version " << version << " (min: " << CLIENT_VERSION_MIN << ", max: " << CLIENT_VERSION_MAX << ")" << std::endl;
    ...
    if(name.empty())
    {
        std::cerr << "[GAME] Empty account name, checking account manager..." << std::endl;
        if(!g_config.getBool(ConfigManager::ACCOUNT_MANAGER))
        {
            std::cerr << "[GAME] Account manager disabled, rejecting empty account name" << std::endl;
            disconnectClient(0x14, "Invalid account name.");
            return false;
        }

        std::cerr << "[GAME] Using account manager (account 1)" << std::endl;
        name = "1";
        password = "1";
    }
    ...
    {
        std::cerr << "[GAME] Server not ready, game state: " << g_game.getGameState() << std::endl;
    ...
    {
        std::cerr << "[GAME] Account '" << name << "' not found in database" << std::endl;
        disconnectClient(0x14, "Invalid account name.");
        return false;
    }
    
    std::cerr << "[GAME] Account '" << name << "' found with ID: " << id << std::endl;

    std::string hash, salt;
    if(!IOLoginData::getInstance()->getPassword(id, hash, salt, character) || !encryptTest(salt + password, hash))
    {
        std::cerr << "[GAME] Authentication failed for account '" << name << "' character '" << character << "'" << std::endl;
        disconnectClient(0x14, "Invalid password.");
        return false;
    }
    
    std::cerr << "[GAME] Authentication successful for account '" << name << "' character '" << character << "'" << std::endl;
    ...
    std::cerr << "[GAME] Dispatching login task for character '" << character << "' (account ID: " << id << ")" << std::endl;
...
#ifdef __DEBUG__
    if(z >= MAP_MAX_LAYERS)
        std::cerr << "[Warning - ProtocolGame::canSee] Z-value is out of range!" << std::endl;
#endif
...
void ProtocolGame::sendFYIBox(const std::string& message)
{
    ...
    {
        std::cerr << "[Warning - ProtocolGame::sendFYIBox] Trying to send an empty or too huge message." << std::endl;
        return;
    }

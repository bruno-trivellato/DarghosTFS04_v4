...
void Battleground::removeIdleWaitlistPlayer(uint32_t player_id)
{
    ...
    if(playerIsInWaitlist(player)){
        player->sendPvpChannelMessage("Nenhuma partida foi iniciada nas ultimas 2 horas. Voc� foi removido da fila. Digite novamente \"!bg entrar\" se voc� deseja voltar a fila!", SPEAK_CHANNEL_O);
...
bool Battleground::playerIsInWaitlist(Player* player)
{
    ...
    for(Bg_Waitlist_t::iterator it = waitlist.begin(); it != waitlist.end(); it++)
	{
        ...
        if(waiting == player)
		{
            ...
            player->sendPvpChannelMessage("ALERTA: Foi identificado que sua conex�o possui mais deu um personagem na fila para a proxima Battleground. O uso de multiclient na Battleground n�o � permitido!", SPEAK_CHANNEL_O);
            return true;
        }
    }
...
void Battleground::finish()
{
	if(teamsMap[BATTLEGROUND_TEAM_ONE].points > teamsMap[BATTLEGROUND_TEAM_TWO].points)
        ...
        if(teamsMap[BATTLEGROUND_TEAM_ONE].points == 0)
            ...
            //empates em 1x1 e 2x2 ser� considerado vencedor o time que capturou a ultima bandeira
...
void Battleground::finishByWinner(Bg_Teams_t teamWinner)
{
	...
	for(BgTeamsMap::iterator it = teamsMap.begin(); it != teamsMap.end(); it++)
	{
		for(PlayersMap::iterator it_players = it->second.players.begin(); it_players != it->second.players.end(); it_players++)
		{
			...
			player->sendPvpChannelMessage("Voc� ser� levado ao lugar em que estava em 5 segundos...");
...
bool Battleground::buildTeams()
{
    ...
    //daqui em diante nada pode mudar, a BG ir� come�ar...
    status = PREPARING;
...
void Battleground::callPlayer(uint32_t player_id)
{
    ...
    player->sendPvpChannelMessage("A battleground est� pronta para iniciar! Voc� tem 2 minutos para digitar o comando \"!bg entrar\" para ser enviado a batalha! Boa sorte bravo guerreiro!", SPEAK_CHANNEL_O);
	player->sendFYIBox("A battleground est� pronta para iniciar!\n Voc� tem 2 minutos para digitar o comando \n\"!bg entrar\" para ser enviado a batalha!\n\n Boa sorte bravo guerreiro!");
}
...
void Battleground::start()
{
	...
	for(BgTeamsMap::iterator it = teamsMap.begin(); it != teamsMap.end(); it++)
	{
        for(PlayersMap::iterator it_players = it->second.players.begin(); it_players != it->second.players.end(); it_players++)
		{
			...
			{
				if(player)
					player->sendPvpChannelMessage("Voc� n�o apareceu na battleground no tempo esperado... Voc� ainda pode participar da batalha digitando \"!bg entrar\" novamente.");
...
BattlegrondRetValue Battleground::onPlayerJoin(Player* player)
{
	...
	else if(status == STARTED || status == PREPARING)
	{
        if(player->isInBattleground()){
             player->sendPvpChannelMessage("Voc� j� est� dentro da battleground!");
        }
        else{
			Bg_Teams_t team_id = findTeamIdByPlayer(player);
			...
			if(!team_id)
			{
				//se a bg j� estiver cheia ele � colocado na fila para a proxima bg
                ...
                if(playerIsInMatch(player)){
                    player->sendPvpChannelMessage("S� � permitido 1 personagem por pessoa dentro da Battleground!");
                    return BATTLEGROUND_NO_ERROR;
                }

				//sen�o, (alguem saiu) ele � colocado na bg
			...
			//o jogador estava na fila, portanto j� esta em um time, somente necessario o teleportar para dentro...
			else
			{
				if(player->hasCondition(CONDITION_INFIGHT))
				{
...
bool Battleground::isValidFrag(Bg_PlayerInfo_t* killer_info, Bg_PlayerInfo_t* target_info)
{
	...
	/* se morreu pouco, entao nao h� necessidade de prosseguir*/
	if(temp_list.size() < 3)
		return true;
	...
	/* Finalmente vamos tentar descobrir se o jogador est� dando 'free frag'...
		Se as 3 ultimas coisas que o jogador fez foi morrer ent�o esta frag
		ser� negada, caso exista algum assist ou kill, ent�o � validado
	*/
	...
	for(DeathsEntryList::const_iterator it = temp_list.begin(); it != temp_list.end(); it++, i++)
	{
		...
		{
			/* Se o jogador matou alguem em suas ultimas 3 a��es entao a frag � valida*/
			return true;
		}
		...
		for(AssistsList::iterator ait = deathEntry->assists.begin(); ait != deathEntry->assists.end(); ait++)
		{
			...
			{
				/* Se o jogador tamb�m causou uma assistencia, entao a frag � valida*/
				return true;
			}
		}
		...
		if(i == 3)
		{
			/* Chegamo na 3a a��o e n�o foi encontrado nada al�m de mortes, ent�o paramos por aqui... a frag n�o � validada */
			return false;
		}
	}

	/*
	Haviam menos de 3 a��es, provavelmente inicio de bg ou algo assim... ent�o validaremos a frag mesmo sem kills/assists.
	hipoteticamente	este ponto do codigo nunca ser� acessado, mas vamos previnir ne...
	*/
	return true;
}

...
bool Quests::loadFromXml()
{
	...
	if(!doc)
	{
		std::cerr << "[Warning - Quests::loadFromXml] Cannot load quests file." << std::endl;
	...
	{
		std::cerr << "[Error - Quests::loadFromXml] Malformed quests file." << std::endl;
		xmlFreeDoc(doc);
		return false;
	}
		...
		if(Mission* mission = new Mission(missionName, missionState, storageId, startValue, endValue))
		{
			if(missionState.empty())
			{
				...
				for(xmlNodePtr stateNode = missionNode->children; stateNode; stateNode = stateNode->next)
				{
					...
					{
						std::cerr << "[Warning - Quests::parseQuestNode] Missing missionId for mission state" << std::endl;
						continue;
					}

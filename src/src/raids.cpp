...
bool Raids::parseRaidNode(xmlNodePtr raidNode, bool checkDuplicate, FileType_t pathing)
{
	...
	{
		std::cerr << "[Error - Raids::parseRaidNode] name tag missing for raid." << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Error - Raids::parseRaidNode] interval2 tag missing or divided by 0 for raid " << name << std::endl;
		return false;
	}
	...
	{
		file = name + ".xml";
		std::cerr << "[Warning - Raids::parseRaidNode] file tag missing for raid " << name << ", using default: " << file << std::endl;
	}
	else
		file = strValue;
	...
	if(!readXMLInteger(raidNode, "margin", intValue))
		std::cerr << "[Warning - Raids::parseRaidNode] margin tag missing for raid " << name << ", using default: " << margin << std::endl;
	else
	...
	if(readXMLString(raidNode, "reftype", strValue) || readXMLString(raidNode, "refType", strValue))
	{
		...
		if(tmpStrValue == "single")
			...
			std::cerr << "[Warning - Raids::parseRaidNode] Unknown reftype \"" << strValue << "\" for raid " << name << std::endl;
	}
	...
	{
		delete raid;
		std::cerr << "[Fatal - Raids::parseRaidNode] failed to load raid " << name << std::endl;
		return false;
	}
...
bool Raids::loadFromXml()
{
	...
	if(!doc)
	{
		std::cerr << "[Warning - Raids::loadFromXml] Could not load raids file." << std::endl;
	...
	{
		std::cerr << "[Error - Raids::loadFromXml] Malformed raids file." << std::endl;
		xmlFreeDoc(doc);
		return false;
	}
...
bool Raid::loadFromXml(const std::string& _filename)
{
	...
	if(!doc)
	{
		std::cerr << "[Error - Raid::loadFromXml] Could not load raid file (" << _filename << ")." << std::endl;
		std::cerr << getLastXMLError() << std::endl;
		return false;
	}
	...
	{
		std::cerr << "[Error - Raid::loadFromXml] Malformed raid file (" << _filename << ")." << std::endl;
		xmlFreeDoc(doc);
		return false;
	}
	...
	while(eventNode)
	{
		...
		{
			std::cerr << "[Error - Raid::loadFromXml] Could not configure raid in file: " << _filename << ", eventNode: " << eventNode->name << std::endl;
			delete event;
		}
		else
...
bool AnnounceEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if(readXMLString(eventNode, "type", strValue))
	{
		...
		if(tmpStrValue == "warning")
			...
			std::cerr << "[Notice - AnnounceEvent::configureRaidEvent] Unknown type tag for announce event, using default: "
				<< (int32_t)m_messageType << std::endl;
	}
	else
		std::cerr << "[Notice - AnnounceEvent::configureRaidEvent] Missing type tag for announce event. Using default: "
			<< (int32_t)m_messageType << std::endl;
...
bool EffectEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if(!readXMLInteger(eventNode, "id", intValue))
	{
		...
		{
			std::cerr << "[Error - EffectEvent::configureRaidEvent] id (or name) tag missing for effect event." << std::endl;
			return false;
		}
		else
	...
	if(!readXMLString(eventNode, "pos", strValue))
	{
		...
		{
			std::cerr << "[Error - EffectEvent::configureRaidEvent] x tag missing for effect event." << std::endl;
			return false;
		}
		...
		{
			std::cerr << "[Error - EffectEvent::configureRaidEvent] y tag missing for effect event." << std::endl;
			return false;
		}
		...
		{
			std::cerr << "[Error - EffectEvent::configureRaidEvent] z tag missing for effect event." << std::endl;
			return false;
		}
	...
	else
	{
		...
		if(posList.size() < 3)
		{
			std::cerr << "[Error - EffectEvent::configureRaidEvent] Malformed pos tag for effect event." << std::endl;
			return false;
		}
...
bool ItemSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if(!readXMLInteger(eventNode, "id", intValue))
	{
		if(!readXMLString(eventNode, "name", strValue))
		{
			std::cerr << "[Error - ItemSpawnEvent::configureRaidEvent] id (or name) tag missing for itemspawn event." << std::endl;
			return false;
		}
		else
	...
	if(!readXMLString(eventNode, "pos", strValue))
	{
		if(!readXMLInteger(eventNode, "x", intValue))
		{
			std::cerr << "[Error - ItemSpawnEvent::configureRaidEvent] x tag missing for itemspawn event." << std::endl;
			return false;
		}
		...
		if(!readXMLInteger(eventNode, "y", intValue))
		{
			std::cerr << "[Error - ItemSpawnEvent::configureRaidEvent] y tag missing for itemspawn event." << std::endl;
			return false;
		}
		...
		if(!readXMLInteger(eventNode, "z", intValue))
		{
			std::cerr << "[Error - ItemSpawnEvent::configureRaidEvent] z tag missing for itemspawn event." << std::endl;
			return false;
		}
	...
	else
	{
		...
		if(posList.size() < 3)
		{
			std::cerr << "[Error - ItemSpawnEvent::configureRaidEvent] Malformed pos tag for itemspawn event." << std::endl;
			return false;
		}
...
bool ItemSpawnEvent::executeEvent() const
{
	...
	if(!tile)
	{
		std::cerr << "[Fatal - ItemSpawnEvent::executeEvent] Missing tile at position " << m_position << std::endl;
		return false;
	}
	...
	if(it.stackable && m_subType > 100)
	{
		...
		while(subCount > 0)
		{
			...
			if(!newItem)
			{
				std::cerr << "[Error - ItemSpawnEvent::executeEvent] Cannot create item with id " << m_itemId << std::endl;
				return false;
			}
			...
			if(ret != RET_NOERROR)
			{
				std::cerr << "[Error - ItemSpawnEvent::executeEvent] Cannot spawn item with id " << m_itemId << std::endl;
				return false;
			}
...
bool SingleSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	else
	{
		...
		if(posList.size() < 3)
		{
			std::cerr << "[Error - SingleSpawnEvent::configureRaidEvent] Malformed pos tag for singlespawn event." << std::endl;
			return false;
		}
...
bool SingleSpawnEvent::executeEvent() const
{
	...
	if(!monster)
	{
		std::cerr << "[Error - SingleSpawnEvent::executeEvent] Cannot create monster " << m_monsterName << std::endl;
		return false;
	}
	...
	{
		delete monster;
		std::cerr << "[Error - SingleSpawnEvent::executeEvent] Cannot spawn monster " << m_monsterName << std::endl;
		return false;
	}
...
bool AreaSpawnEvent::configureRaidEvent(xmlNodePtr eventNode)
{
	...
	if(readXMLInteger(eventNode, "radius", intValue))
	{
		...
		if(readXMLString(eventNode, "centerPosition", strValue) || readXMLString(eventNode, "centerpos", strValue))
		{
			...
			if(posList.size() < 3)
			{
				std::cerr << "[Error - AreaSpawnEvent::configureRaidEvent] Malformed centerPosition tag for areaspawn event." << std::endl;
				return false;
			}
	...
	else
	{
		if(readXMLString(eventNode, "fromPosition", strValue) || readXMLString(eventNode, "frompos", strValue))
		{
			...
			if(posList.size() < 3)
			{
				std::cerr << "[Error - AreaSpawnEvent::configureRaidEvent] Malformed fromPosition tag for areaspawn event." << std::endl;
				return false;
			}
		...
		if(readXMLString(eventNode, "toPosition", strValue) || readXMLString(eventNode, "topos", strValue))
		{
			...
			if(posList.size() < 3)
			{
				std::cerr << "[Error - AreaSpawnEvent::configureRaidEvent] Malformed toPosition tag for areaspawn event." << std::endl;
				return false;
			}
...
bool AreaSpawnEvent::executeEvent() const
{
	for(MonsterSpawnList::const_iterator it = m_spawnList.begin(); it != m_spawnList.end(); it++)
	{
		...
		for(uint32_t i = 0; i < amount; ++i)
		{
			...
			if(!monster)
			{
				std::cerr << "[Error - AreaSpawnEvent::executeEvent] Cannot create monster " << spawn->name << std::endl;
				return false;
			}
...
bool ScriptEvent::configureRaidEvent(xmlNodePtr eventNode)
{
    ...
    if(!m_interface.loadFile(getFilePath(FILE_TYPE_OTHER, std::string(scriptsName + "/lib/" + scriptsName + ".lua"))))
		std::cerr << "[Warning - ScriptEvent::configureRaidEvent] Cannot load " << scriptsName << "/lib/" << std::endl;
	...
	if(readXMLString(eventNode, "file", strValue))
	{
		...
		{
			std::cerr << "[Error - ScriptEvent::configureRaidEvent] Cannot load raid script file (" << strValue << ")." << std::endl;
			return false;
		}
	}
	...
	std::cerr << "[Error - ScriptEvent::configureRaidEvent] Cannot load raid script buffer." << std::endl;
	return false;
}

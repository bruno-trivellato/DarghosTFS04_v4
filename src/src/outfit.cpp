...
bool Outfits::parseOutfitNode(xmlNodePtr p)
{
	...
	{
		std::cerr << "[Error - Outfits::parseOutfitNode] Missing outfit id, skipping" << std::endl;
		return false;
	}
	...
	for(xmlNodePtr listNode = p->children; listNode != NULL; listNode = listNode->next)
	{
		...
		{
			std::cerr << "[Error - Outfits::parseOutfitNode] Missing looktype for an outfit with id " << outfit.outfitId << std::endl;
			continue;
		}
		...
		{
			std::cerr << "[Error - Outfits::parseOutfitNode] Missing gender(s) for an outfit with id " << outfit.outfitId
		...
		{
			std::cerr << "[Error - Outfits::parseOutfitNode] Invalid gender(s) for an outfit with id " << outfit.outfitId
				<< " and looktype " << outfit.lookType << std::endl;
			continue;
		}
		...
		if(readXMLString(listNode, "requirement", strValue))
		{
			...
			if(tmpStrValue == "none")
				...
				std::cerr << "[Warning - Outfits::loadFromXml] Unknown requirement tag value, using default (both)" << std::endl;
		}
		...
		for(IntegerVec::iterator it = intVector.begin(); it != intVector.end(); ++it)
		{
			...
			if(fit != outfitsMap[(*it)].end())
			{
				if(override)
				{
					...
					std::cerr << "[Warning - Outfits::parseOutfitNode] Duplicated outfit for gender " << (*it) << " with lookType " << outfit.outfitId << std::endl;
			}
			else
			{
...
bool Outfits::loadFromXml()
{
	...
	if(!doc)
	{
		std::cerr << "[Warning - Outfits::loadFromXml] Cannot load outfits file, using defaults." << std::endl;
	...
	{
		std::cerr << "[Error - Outfits::loadFromXml] Malformed outfits file." << std::endl;
		xmlFreeDoc(doc);
		return false;
	}

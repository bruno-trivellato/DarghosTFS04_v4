...
DELETE FROM accounts WHERE name IN ('bruno', 'ninao', 'yves', 'rafa');
...
(NULL,'rafa',SHA1('rafa'),'',365,UNIX_TIMESTAMP(),'rafa@darghos.com','0',0,0,1,0,0);
...
SET @rafa_id = (SELECT id FROM accounts WHERE name = 'rafa');
...
-- Rafa's characters
(NULL,'Rafa Knight',0,1,@rafa_id,8,4,185,185,4200,68,76,78,58,131,0,0,35,35,0,100,1,95,117,7,_binary '',470,1,0,0,1,0,0,0,'',0,31,0,151200000,2,100,100,100,100,100,0,0,0,0,0,'',0,1,0),
(NULL,'Rafa Sorc',0,1,@rafa_id,200,1,1400,1400,364619811,113,115,95,39,138,0,100,8000,8000,500000000,100,1,95,117,7,_binary '',430,1,0,0,1,0,0,0,'',0,31,0,151200000,2,100,100,100,100,100,0,0,0,0,0,'',0,1,0),
(NULL,'GOD Rafa',0,9,@rafa_id,500,0,50000,50000,2058474800,0,0,0,0,302,0,200,50000,50000,0,200,1,1997,1849,7,_binary '',50000,1,0,0,1,0,0,0,'',0,31,0,151200000,2,100,100,100,100,100,0,0,0,0,0,'',0,1,0);
...
WHERE p.account_id IN (@bruno_id, @ninao_id, @yves_id, @rafa_id);
...
WHERE a.name IN ('bruno', 'ninao', 'yves', 'rafa')
GROUP BY a.name;
